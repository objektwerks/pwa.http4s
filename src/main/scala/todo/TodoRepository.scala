...
class TodoRepository(xa: Transactor[IO], schema: String) {
  ...
  def insert(todo: Todo): Id = Id(insertTodo.toUpdate0((todo.text, todo.opened, todo.closed))
                               .withUniqueGeneratedKeys[Int]("id").transact(xa).unsafeRunSync)

  def update(todo: Todo): Count = Count(updateTodo.toUpdate0((todo.text, todo.closed, todo.id))
...
object TodoRepository {
  ...
  val insertTodo = Update[(String, Timestamp, Timestamp)]("insert into todo(text, opened, closed) values (?, ?, ?)")
  val updateTodo = Update[(String, Timestamp, Int)]("update todo set text = ?, closed = ? where id = ?")
